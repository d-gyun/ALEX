...
class RMI:
    def __init__(self, reg, data):
        ...
        self.root = LearnedIndexNode(reg, 0, self.data, len(self.data),0)
        ...
        while self.nodeSize > self.leafNodeSize:
            ...
            for i in range(0, len(self.data)//self.interval + (1 if (len(self.data)%self.interval) else 0)):
                if self.interval*(i+1) < len(self.data):
                    childNode = LearnedIndexNode(reg, self.stageIdx[-1], self.data[self.interval*i:self.interval*(i+1)], self.interval,idx=i)
                else:
                    childNode = LearnedIndexNode(reg, self.stageIdx[-1], self.data[self.interval*i:], self.interval, idx=i)
...
class LearnedIndexNode:
    def __init__(self, reg, stage, data, interval, idx):
        self.reg = reg
        self.stage = stage
        self.model = None
        self.index = None
        # print(self.data)
        self.labels = np.where(~np.isnan(data))[0] + int(interval)*idx  #Create Labels
